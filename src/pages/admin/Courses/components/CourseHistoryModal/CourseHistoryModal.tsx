...
import { useGetCourseHistoriesQuery } from '../../course.service';
...
const CourseHistoryModal: React.FC<CourseHistoryModalProps> = ({ courseId, isOpen, onClose }) => {
  const [currentPage, setCurrentPage] = useState(1);
  const [pageSize, setPageSize] = useState(10);

  const { data: historyData, isFetching: isHistoryFetching } = useGetCourseHistoriesQuery({
    courseId,
  ...
  return (
    <Modal title='Coupon History' open={isOpen} onCancel={onClose} footer={null} width={1500}>
      <Table
